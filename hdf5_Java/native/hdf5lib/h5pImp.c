...
{
    char *ceph_conf_path_char;
    herr_t retVal = -1;

    // convert the Java String into a char*
    ceph_conf_path_char = (char *)(*env)->GetStringUTFChars(env, ceph_conf_path, NULL);
    if (NULL == ceph_conf_path_char)  {
        h5libraryError(env);
    }

    retVal =  H5Pset_fapl_ceph((hid_t)fapl_id, ceph_conf_path_char);
    if (retVal < 0) {
        h5libraryError(env);
    }

    // release the Java String reference 
    (*env)->ReleaseStringUTFChars(env, ceph_conf_path, ceph_conf_path_char);
