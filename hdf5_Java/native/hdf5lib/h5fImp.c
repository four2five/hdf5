...
/*
JNIEXPORT jint JNICALL Java_ncsa_hdf_hdf5lib_H5_loadGlobalLibraries
  (JNIEnv *env, jclass obj)
{
    lt_dladvise advise;

    if (lt_dlinit() != 0) {
        h5badArgument(env, "LT_DLINIT FAILED - cannot load libjhdf5.so");
        return -1;
    }

    // open the library into the global namespace 
    if (lt_dladvise_init(&advise)) {
        h5badArgument(env, "LT_DLADVISE INIT FAILED - cannot load libjhdf5.so");
        return -2;
    }

    if (lt_dladvise_ext(&advise)) {
        h5badArgument(env, "LT_DLADVISE EXT FAILED - cannot load libjhdf5.so");
        lt_dladvise_destroy(&advise);
        return -3;
    }

    if (lt_dladvise_global(&advise)) {
        h5badArgument(env, "LT_DLADVISE GLOBAL FAILED - cannot load libjhdf5.so");
        lt_dladvise_destroy(&advise);
        return -4;
    }
    ...
    //
    lt_dlopenadvise("libjhdf5", advise);
    lt_dlopenadvise("libhdf5", advise);
    lt_dladvise_destroy(&advise);
