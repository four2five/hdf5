...
/*
 * Class:     ncsa_hdf_hdf5lib_H5
 * Method:    H5Dget_offsets
 * Signature: (IIII[JZ)I
 */
JNIEXPORT jint JNICALL Java_ncsa_hdf_hdf5lib_H5_H5Dget_1offsets
  ...
  jint file_space_id, jint xfer_plist_id, jlongArray buf, jboolean isCriticalPinning)
{
    ...
    if((data_class == 1) || (data_class < 0)) {
        h5badArgument( env, "H5Dget_offsets:  buf does not support variable length type");
        return -1;
    }
    /* recursive detect any vlen string in type (compound, array ...) */
    data_class = H5Tdetect_variable_str(mem_type_id);
    if((data_class == 1) || (data_class < 0)) {
        h5badArgument( env, "H5Dget_offsets:  buf does not support variable length string type");
        return -1;
    }

    if ( buf == NULL ) {
        h5nullArgument( env, "H5Dget_offsets:  buf is NULL");
        return -1;
    }
    ...
    if (buffP == NULL) {
        h5JNIFatalError( env, "H5Dget_offsets:  buf not pinned");
        return -1;
    }

    status = H5Dget_offsets((hid_t)dataset_id, (hid_t)mem_type_id, 
