...
static ssize_t H5D_contig_get_offsetsvv(const H5D_io_info_t *io_info,
    size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[], 
    hsize_t mem_off_arr[], size_t *write_buf_off, size_t elmt_size);
...
const H5D_layout_ops_t H5D_LOPS_CONTIG[1] = {{
    ...
    H5D_contig_flush,
    NULL,
    H5D_contig_get_offsets, /*ser_get_offsets*/
    H5D_contig_get_offsetsvv, /*get_offsetsvv*/
}};
 ...
 */
herr_t
H5D_contig_get_offsets(H5D_io_info_t *io_info, const H5D_type_info_t *type_info,
    hsize_t nelmts, const H5S_t *file_space, H5D_chunk_map_t UNUSED *fm)
{
    herr_t	ret_value = SUCCEED;	/*return value		*/

    FUNC_ENTER_NOAPI_NOINIT(H5D_contig_get_offsets)
    /* Sanity check */
    HDassert(io_info);
    ...
    /* Read data */
    if((io_info->io_ops.get_offsets)(io_info, type_info, nelmts, file_space) < 0)
...
H5D_contig_readvv_cb(hsize_t dst_off, hsize_t src_off, size_t len, void *_udata)
{
    ...
    /* Write data */
    printf("\tin _cb, buffer offset is %p\n", udata->rbuf); 
    printf("\tin _cb, buffer offset + src_off is %p\n", udata->rbuf + src_off); 
...
//H5D_contig_get_offsets_cb(hsize_t dst_off, hsize_t src_off, size_t len, void *_udata)
static herr_t
H5D_contig_get_offsets_sieve_cb(hsize_t src_off, size_t len, size_t *write_buf_off, 
                                size_t elmt_size, void *_udata)
{
    /* User data for H5V_opvv() operator */
    H5D_contig_readvv_sieve_ud_t *udata = (H5D_contig_readvv_sieve_ud_t *)_udata; 
    const H5D_contig_storage_t *store_contig = udata->store_contig;

    int i = 0;
    int num_offsets_to_set ;

    herr_t ret_value = SUCCEED;         /* Return value */

    FUNC_ENTER_NOAPI_NOINIT(H5D_contig_get_offsets_sieve_cb)

    // get datatype size
    //printf("dset_addr %llu\n", store_contig->dset_addr);

    src_off += store_contig->dset_addr;
    // figure out how many data elements are represented by this sequence
    num_offsets_to_set = len / elmt_size;

    // for each data element, write it's offset (which is a hsize_t)
    // into the output buffer. The iterate the src_off the size of a 
    // data element and repeat
    for ( i = 0; i < num_offsets_to_set; i++) { 
      HDmemcpy(udata->rbuf + *write_buf_off, &src_off, sizeof(hsize_t)); 
      *write_buf_off += sizeof(hsize_t);
      src_off += elmt_size;
    }
    //*(udata->rbuf) = src_off;
    //advance the receiving buffer's pointer
    //_udata = _udata + sizeof(hsize_t);
...
static herr_t
H5D_contig_get_offsets_cb(hsize_t src_off, size_t len, size_t *write_buf_off, 
                          size_t elmt_size, void *_udata)
{
    H5D_contig_readvv_ud_t *udata = (H5D_contig_readvv_ud_t *)_udata; /* User data for H5V_opvv() operator */
    herr_t ret_value = SUCCEED;         /* Return value */

    FUNC_ENTER_NOAPI_NOINIT(H5D_contig_get_offsets_cb)

    printf("\tin _cb, buffer offset is %p\n", udata->rbuf); 
    // copy the offset into the receiving buffer
    printf("\tin _cb, writing %llu into udata->rbuf\n", src_off);
    printf("\tin _cb, write_buf_off is %d\n", *write_buf_off);
 ...
 */
static ssize_t
H5D_contig_get_offsetsvv(const H5D_io_info_t *io_info, size_t mem_max_nseq, 
                         size_t *mem_curr_seq,  size_t mem_len_arr[], 
                         hsize_t mem_off_arr[], size_t *write_buf_off,
                         size_t elmt_size)
{
    ssize_t ret_value;          /* Return value */
    FUNC_ENTER_NOAPI_NOINIT(H5D_contig_get_offsetsvv)
    ...
    //printf("in H5D_contig_get_offsetsvv, elmt_size %d\n", elmt_size);

    /* Check if data sieving is enabled */
    if(H5F_HAS_FEATURE(io_info->dset->oloc.file, H5FD_FEAT_DATA_SIEVE)) {
        H5D_contig_readvv_sieve_ud_t udata;     /* User data for H5V_opv() operator */
        ...
        if((ret_value = H5V_opv( mem_max_nseq, mem_curr_seq, mem_len_arr, mem_off_arr,
                write_buf_off, elmt_size, H5D_contig_get_offsets_sieve_cb, &udata)) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL, "can't perform vectorized sieve buffer get offsets")
    } /* end if */
    else {
        H5D_contig_readvv_ud_t udata;     /* User data for H5V_opv() operator */
        ...
        /* Call generic sequence operation routine */
        if((ret_value = H5V_opv(  mem_max_nseq, mem_curr_seq, mem_len_arr, mem_off_arr,
                write_buf_off, elmt_size, H5D_contig_get_offsets_cb, &udata)) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTOPERATE, FAIL, "can't perform vectorized get offsets")
    } /* end else */

done:
    FUNC_LEAVE_NOAPI(ret_value)
}   /* end H5D_contig_get_offsetsvv() */
    ...
    size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[], hsize_t mem_off_arr[])
{
    ...
    if(H5F_HAS_FEATURE(io_info->dset->oloc.file, H5FD_FEAT_DATA_SIEVE)) {
        ...
        /* Set up user data for H5V_opvv() */
        printf("in contig_readvv, u.buf %p\n", io_info->u.rbuf);
        udata.file = io_info->dset->oloc.file;
        udata.dset_addr = io_info->store->contig.dset_addr;
        udata.rbuf = (unsigned char *)io_info->u.rbuf;
        udata.dxpl_id = io_info->dxpl_id;
        printf("in contig_readvv, udata.rbuf %p\n", udata.rbuf);
