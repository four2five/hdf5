...
static herr_t
H5D_select_get_offsets_io(const H5D_io_info_t *io_info, size_t elmt_size, size_t nelmts, 
    ...
    ssize_t tmp_off_len;       /* Temporary number of bytes in file sequence */
    size_t write_buffer_offset; /* offset into the buffer being written */
    herr_t ret_value = SUCCEED; /* Return value */

    FUNC_ENTER_NOAPI(H5D_select_get_offsets_io, FAIL)
    //printf("in H5D_select_get_offsets_io, elmt_size:%d\n", elmt_size);
    HDassert(io_info);
    ...
    if(nelmts == 1) {
        ...
        *file_len = elmt_size;
        ...
        //ret_value = file_len; // ????
        if ((tmp_off_len = (*io_info->layout_ops.get_offsetsvv)(io_info,
              file_nseq, &curr_file_seq, file_len, file_off, 
              &write_buffer_offset, elmt_size )) < 0)
                HGOTO_ERROR(H5E_DATASPACE, H5E_READERROR, FAIL, "get offset error")
        /* Perform I/O on memory and file sequences */
        /*
            printf("file_len: %d file_off: %d mem_len: %d mem_off: %d\n", 
                    *file_len, *file_off, *mem_len, *mem_off);
        ...
        */
        ...
        while(nelmts > 0) {
            ...
            if(curr_file_seq >= file_nseq) {
                ...
                if(H5S_SELECT_GET_SEQ_LIST(file_space, H5S_GET_SEQ_LIST_SORTED, &file_iter, io_info->dxpl_cache->vec_size, 
                   nelmts, &file_nseq, &file_nelem, file_off, file_len) < 0)
            ...
            HDassert(((size_t)tmp_off_len % elmt_size) == 0);
            nelmts -= ((size_t)tmp_off_len / elmt_size);
        } /* end while */
    } /* else for if(nelmts == 1) */
done:
...
herr_t
prime_offsets(const H5D_io_info_t *io_info, const H5D_type_info_t *type_info,
              hsize_t nelmts)
{
  int i = 0;
  hsize_t fakeVal = 3;
  int elmt_size = type_info-> src_type_size;
  for ( i = 0; i < nelmts; i++ ) { 
    HDmemcpy(io_info->u.rbuf + (i * sizeof(hsize_t)), &fakeVal, sizeof(hsize_t));
  }

  printf("done priming, here's the array contents at the moment\n");
  printf("sizeof hsize_t %d\n", sizeof(hsize_t));
  for( i = 0; i < nelmts; i++ ) { 
    if( i > 0 )   
      printf(",");
    printf(" %llu\n", io_info->u.rbuf + (i * sizeof(hsize_t)) );
  }

}

herr_t
H5D_select_get_offsets(const H5D_io_info_t *io_info, const H5D_type_info_t *type_info,
    hsize_t nelmts, const H5S_t *file_space )
{
    herr_t ret_value = SUCCEED; /* Return value */

    FUNC_ENTER_NOAPI(H5D_select_get_offsets, FAIL)

    //prime_offsets(io_info, type_info, nelmts);

    /* Call generic selection operation */
    H5_CHECK_OVERFLOW(nelmts, hsize_t, size_t);
    //if(H5D_select_get_offsets_io(io_info, type_info->src_type_size, (size_t)nelmts,
    //        file_space) < 0)

    // I *think* we want to use size(hsize_t) as the element size,
    // since that's what each offset we're setting is (in terms of bytes)
    if(H5D_select_get_offsets_io(io_info, type_info->src_type_size, (size_t)nelmts,
            file_space) < 0)
        HGOTO_ERROR(H5E_DATASPACE, H5E_READERROR, FAIL, "read error")
