...
/**
 * a function that simply iterates over a single sequency of offset/length
 * pairs. At present, only used for getting the offsets for points in a 
 * dataset
*/
ssize_t
H5V_opv(  size_t src_max_nseq, size_t *src_curr_seq, size_t src_len_arr[],
    hsize_t src_off_arr[], size_t *write_buf_off, size_t elmt_size,
    H5V_opv_func_t op, void *op_data)
{
    hsize_t *max_src_off_ptr;  /* Pointers to max. source offset locations */
    hsize_t *src_off_ptr; /* Pointers to source offset arrays */
    size_t  *src_len_ptr;  /* Pointers to source length arrays */
    hsize_t tmp_src_off;   /* Temporary source offset values */
    size_t  tmp_src_len;    /* Temporary destination length values */
    size_t acc_len;             /* Accumulated length of sequences */

    // this is silly, but seems neccesary

    ssize_t ret_value = 0;      /* Return value (Total size of sequence in bytes) */

    FUNC_ENTER_NOAPI(H5V_opv, FAIL)
    ...
    /* Get temporary source sequence offsets & lengths */
    ...
    do {
        ...
        if((*op)(tmp_src_off, tmp_src_len, write_buf_off, elmt_size,
                 op_data) < 0)
            HGOTO_ERROR(H5E_INTERNAL, H5E_CANTOPERATE, FAIL, "can't perform operation")

        // trial print
        // try putting the data into the buffer
        //(op_data->rbuf + acc_len) = tmp_src_off;

        /* Accumulate number of bytes copied */
        //acc_len += tmp_src_len;
        //acc_len += sizeof(op_data->rbuf);

        //we shuld just increment the size of an hsize_t value
        //acc_len += sizeof(hsize_t);

        // we need to figure out how many elements this call 
        // covered, and then increment the equivalent number
        // of pointers
        acc_len += tmp_src_len;
    ...
    } while(src_off_ptr < max_src_off_ptr); // redundanct with the goto above but what to do?
