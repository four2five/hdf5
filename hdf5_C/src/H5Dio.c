...
static herr_t H5D_write(H5D_t *dataset, hid_t mem_type_id,
    const H5S_t *mem_space, const H5S_t *file_space, hid_t dset_xfer_plist,
    const void *buf);
static herr_t H5D_get_offsets(H5D_t *dataset, hid_t mem_type_id,
 ...
 */
herr_t
H5Dget_offsets(hid_t dset_id, hid_t mem_type_id,
	hid_t file_space_id, hid_t plist_id, void *buf/*out*/)
{
    ...
    FUNC_ENTER_API(H5Dget_offsets, FAIL)
    H5TRACE5("e", "iiiix", dset_id, mem_type_id, file_space_id, plist_id, buf);
    ...
    */
    if (H5D_get_offsets(dset, mem_type_id, file_space, plist_id, buf /*out*/) < 0)
	    HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "can't get offsets")
 ...
 */
static herr_t
H5D_get_offsets(H5D_t *dataset, hid_t mem_type_id, const H5S_t *file_space, 
                hid_t dxpl_id, void *buf/*out*/)
{
    ...
    FUNC_ENTER_NOAPI_NOINIT(H5D_get_offsets)
    ...
    if((snelmts = H5S_GET_SELECT_NPOINTS(file_space)) < 0)
    ...
    /* trying to pass file_space as mem_space as a placebo */
    if(io_info.layout_ops.io_init && (*io_info.layout_ops.io_init)(&io_info, &type_info, nelmts, file_space, 
       file_space, &fm) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't initialize I/O info")
    io_op_init = TRUE;

    /* Invoke correct "high level" I/O routine */
    if((*io_info.io_ops.multi_get_offsets)(&io_info, &type_info, nelmts, file_space, &fm) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "can't get the offsets")
    /*
    ...
    const H5D_type_info_t *type_info, H5D_storage_t *store, H5D_io_info_t *io_info)
{
    ...
    io_info->io_ops.multi_get_offsets = dset->shared->layout.ops->ser_get_offsets;
    io_info->io_ops.get_offsets = H5D_select_get_offsets;
